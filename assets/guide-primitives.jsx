...
function AutomationFunnelMock() {
  ...
  return (
    <Mock title="Automation · Statin Initiation Funnel" caption="Members fall through each step until the care gap closes; remainder is handed to a clinical user">
      <div style={{ display: "grid", gridTemplateColumns: `repeat(${steps.length}, 1fr) 28px`, gap: 6, alignItems: "stretch" }}>
        {steps.map((s, i) => (
          <Fragment key={i}>
            <div style={{
              padding: 14,
              background: i === 3 ? "var(--od-primary-800)" : "var(--g-card-bg)",
              color: i === 3 ? "#FFF" : "inherit",
              border: i === 3 ? "0" : "1px solid var(--g-border)",
              borderRadius: 8,
              display: "flex", flexDirection: "column", gap: 4,
            }}>
              <div style={{ fontSize: 10, letterSpacing: ".1em", textTransform: "uppercase", fontWeight: 700, color: i === 3 ? "var(--g-sky)" : "var(--g-fg-3)" }}>Step {i + 1}</div>
              <div style={{ fontWeight: 700, fontSize: 13 }}>{s.t}</div>
              <div style={{ fontSize: 11, opacity: 0.7 }}>to {s.sub}</div>
              <div style={{ marginTop: 6, display: "flex", justifyContent: "space-between", alignItems: "baseline", fontSize: 11 }}>
                <span style={{ opacity: 0.6 }}>pause {s.pause}</span>
                <span style={{ fontWeight: 700, fontFamily: "var(--g-mono)" }}>{s.live}</span>
              </div>
            </div>
            {i < steps.length - 1 ? (
              <i className="fa-solid fa-arrow-right-long" style={{ alignSelf: "center", color: "var(--g-fg-4)", fontSize: 14 }}></i>
            ) : null}
          </Fragment>
        ))}
      </div>
...
function ClairMock() {
  return (
    <Mock title="Clair Co-pilot · AI Patient Insights" caption="Triggered from a patient page or table row · references attached to every claim">
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <div style={{ padding: 14, background: "linear-gradient(135deg, #2D1B5C 0%, #5B2D9E 100%)", color: "#FFF", borderRadius: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 12, opacity: 0.7, letterSpacing: ".1em", textTransform: "uppercase", fontWeight: 700 }}>Clair</div>
